...
pool_fit_batch <- function(upper, selector, file) {
  if (!file.exists(file)) {
    pool_fit_results <- tibble(n_pts = integer(), n_ols = integer(), loss = num(notation = "sci"), time = double())
    for (n in 2:upper) {
...
ds_train <- generator(n_vec = 4096, d = 2, n_categories = 2)
ds_test <- generator(n_vec = 2048, d = 2, n_categories = 1)
plot_dataset(ds_train, ds_test, n_categories = 2)
ggsave("q3_cat2_train4096_test2048_np.svg")
```
...
ggsave("q3_cat2_train4096_test2048_k1.svg")
...
ggsave("q3_cat2_train4096_test2048_k3.svg")
```

## One more complicated dataset with 4 categories

```{r}
ds_train <- generator(n_vec = 2048, d = 2, n_categories = 4)
ds_test <- generator(n_vec = 1024, d = 2, n_categories = 1)
ds_test_pred <- tibble(x = ds_test$x, y = knn_classifier(ds_train$x, ds_train$y, ds_test$x, k = 3))
plot_dataset(ds_train, ds_test_pred, n_categories = 4, k = 4)
ggsave("q3_cat4_train2048_test1024_k4.svg")
```
